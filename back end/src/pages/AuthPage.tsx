...
import TelegramBotLoginModal from "@/components/TelegramBotLoginModal";
...
export default function AuthPage() {
  ...
  const handleVerifyForgotOtp = () => {
    if (otpCode.length !== 4) return;
    // Код проверяет и «сжигает» сам reset-password, поэтому здесь
    // мы только переходим на следующий экран, сохраняя otpCode.
  ...
  const formContent = (
    <div className="w-full max-w-[400px]">
      ...
      {/* Telegram Login Buttons */}
      <div className="mb-6 space-y-2.5">
        <button
          onClick={() => setBotLoginOpen(true)}
          className="w-full flex items-center justify-center gap-2.5 bg-[#2AABEE] hover:bg-[#229ED9] text-white font-semibold py-3.5 rounded-xl transition-colors shadow-sm"
        >
        ...
        {botUsername && (
          <button
            ...
            disabled={tgLoading || !botId}
            className="w-full flex items-center justify-center gap-2 text-xs text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50"
          >
            {tgLoading ? "Авторизация…" : "Использовать старый виджет Telegram"}
          </button>
        )}
      </div>
  ...
  return (
    <>
    <TelegramBotLoginModal open={botLoginOpen} onClose={() => setBotLoginOpen(false)} />
